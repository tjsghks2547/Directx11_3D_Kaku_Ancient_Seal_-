...
void  CHitEffect::Priority_Update(_float fTimeDelta)
{
...
void  CHitEffect::Update(_float fTimeDelta)
{

}

void  CHitEffect::Late_Update(_float fTimeDelta)
{
    /* 여기서 버퍼 업데이트 해줘야겠다. */
...
void CHitEffect::OnCollisionEnter(CGameObject* _pOther)
{
}
...
void CHitEffect::OnCollisionExit(CGameObject* _pOther)
{
}

HRESULT CHitEffect::Ready_Components()
{
    /* Com_Model */
    ...
    /* Com_Texture */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_HitEffect0"),       
        TEXT("Com_Texture0"), reinterpret_cast<CComponent**>(&m_pTextureCom0))))      
        return E_FAIL;

    /* Com_Texture */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_HitEffect1"),
        TEXT("Com_Texture1"), reinterpret_cast<CComponent**>(&m_pTextureCom1))))
        return E_FAIL;
...
HRESULT CHitEffect::Bind_ShaderResources()
{
    ...
    if (FAILED(m_pTextureCom1->Bind_ShaderResource(m_pShaderCom, "g_HitEffect1", 0)))   
        return E_FAIL;

    if (FAILED(m_pTextureCom2->Bind_ShaderResource(m_pShaderCom, "g_HitEffect2", 0)))   
        return E_FAIL;
...
HRESULT CHitEffect::Effect_Activate()
{
...
CHitEffect* CHitEffect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)    
{
    ...
    {
        MSG_BOX("Failed to Created : CHitEffect");  
