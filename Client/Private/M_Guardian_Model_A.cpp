...
CM_Guardian_Model_A::CM_Guardian_Model_A(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CMonster(pDevice, pContext)
{
}

CM_Guardian_Model_A::CM_Guardian_Model_A(const CM_Guardian_Model_A& Prototype)
	:CMonster(Prototype)
{
}

HRESULT CM_Guardian_Model_A::Initialize_Prototype()
{
...
HRESULT CM_Guardian_Model_A::Initialize(void* pArg)
{
	/*if (m_szName == nullptr)
...
void CM_Guardian_Model_A::Priority_Update(_float fTimeDelta)
{
	...
	if (m_bMonsterActive)
	{
		...
		else if (!m_bDead)
		{
			...
			else if (distance < 300.f && AnimationFinish && m_bStart && m_fAccTimeDelta < 12.0f && !m_bAttack)
			{
			...
			else if (distance <= 100.f && distance > 50.f && m_fAccTimeDelta > 12.0f && AnimationFinish && m_bStart && m_iState != STATE_ATTACK_ANTICIPATE)
			{

				m_iState = 0;
				m_iState = STATE_ATTACK_ANTICIPATE;
				...
				_vector vCurPos = m_pTransformCom->Get_State(CTransform::STATE_POSITION);	//= { m_CombinedWorldMatrix._41,m_CombinedWorldMatrix._42,m_CombinedWorldMatrix._43,1.f };	

				CBullet* pBullet = CBullet::Create(m_pDevice, m_pContext);
				if (pBullet == nullptr)
					return;

				pBullet->Set_Poisition(_float4{ vCurPos.m128_f32[0],vCurPos.m128_f32[1],vCurPos.m128_f32[2],1.f });
				pBullet->Set_Socket_Matrix(m_pModelCom->Get_BoneMatrix("Bone001"));
				if (m_pModelCom->Get_BoneMatrix("Bone001") == nullptr)
				{
					int test = 4;
				}
				pBullet->Set_Parent_Matrix(m_pTransformCom->Get_WorldMatrix_Ptr());
				pBullet->Set_Parent_State(&m_iState);
				pBullet->Set_Player_Ptr(m_pPlayer);
				m_pGameInstance->Add_Effect(pBullet);

				m_bAttack = true;


				CBulletGainEffectParticle::PARTICLE_EXPLOSION_DESC EffectDesc{};
				...
				EffectDesc.ParentSocketMarix = m_pModelCom->Get_BoneMatrix("Bone001");
				EffectDesc.pParentState = &m_iState;

				if (FAILED(m_pGameInstance->Add_GameObject_To_Layer(LEVEL_GAMEPLAY, TEXT("Prototype_GameObject_Particle_BulletGainEffect"), LEVEL_GAMEPLAY, TEXT("Layer_Particle"), &EffectDesc)))
				{
					MSG_BOX("Failed to Effect : BulletGainEffect");
				}
			}


			else if (m_bStart && m_bAttack && AnimationFinish)
			{
			...
			else if (distance <= 50.f && m_fAccTimeDelta > 12.0f && AnimationFinish && m_bStart)
			{
				m_iState = 0;
				m_iState = STATE_ROLL;
			...
			if (m_pModelCom->Get_VecAnimation().at(8)->isAniMationFinish())
			{
				m_bAttack = false;
			}
...
void CM_Guardian_Model_A::Update(_float fTimeDelta)
{
	...
	else if (m_iState == STATE_ATTACK_ANTICIPATE)	
	{
		if (m_pNavigationCom->isMove(m_pTransformCom->Get_State(CTransform::STATE_POSITION)))
		{
			//외적 구해야겟네 
...
void CM_Guardian_Model_A::Late_Update(_float fTimeDelta)
{
	if (m_bMonsterActive)
	{
...
HRESULT CM_Guardian_Model_A::Render()
{
...
HRESULT CM_Guardian_Model_A::Render_Shadow()
{
...
HRESULT CM_Guardian_Model_A::Ready_Components()
{
	/* Com_Shader */
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_M_Guardian_Model_A"),
	...
	if (!strcmp(m_szName, "M_Guardian_Model_A_01"))
		Desc.iCurrentCellIndex = 227;
...
void CM_Guardian_Model_A::OnCollisionEnter(CGameObject* _pOther)
{
	_bool AnimationFinish = m_pModelCom->Get_VecAnimation().at(m_pModelCom->Get_Current_Animation_Index())->isAniMationFinish();
...
void CM_Guardian_Model_A::OnCollision(CGameObject* _pOther)
{
	_bool AnimationFinish = m_pModelCom->Get_VecAnimation().at(m_pModelCom->Get_Current_Animation_Index())->isAniMationFinish();
...
void CM_Guardian_Model_A::OnCollisionExit(CGameObject* _pOther)
{
	int a = 4;
}
...
HRESULT CM_Guardian_Model_A::Bind_ShaderResources()
{
	if (FAILED(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix")))
		return E_FAIL;
...
CM_Guardian_Model_A* CM_Guardian_Model_A::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CM_Guardian_Model_A* pInstance = new CM_Guardian_Model_A(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CM_Guardian_Model_A");
...
CGameObject* CM_Guardian_Model_A::Clone(void* pArg)
{
	CM_Guardian_Model_A* pInstance = new CM_Guardian_Model_A(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CM_Guardian_Model_A");
		Safe_Release(pInstance);
	}
