...
void  CS_HpFlower::Late_Update(_float fTimeDelta)
{
    ...
    if (m_isCollision && m_pGameInstance->isKeyEnter(DIK_E))
    {
        m_bIconOnOff = true;
        /* 아이템 추가 코드 */
        m_pGameInstance->Add_Item(TEXT("Prototype_GameObject_S_HpFlower_Ui"));
    }
...
void CS_HpFlower::IsTriggerOn(_wstring _EventName)
{

}

void CS_HpFlower::OnCollisionEnter(CGameObject* _pOther)
{
    
}

void CS_HpFlower::OnCollision(CGameObject* _pOther)
{
    m_isCollision = true;
}

void CS_HpFlower::OnCollisionExit(CGameObject* _pOther)
{
    m_isCollision = false;
}
